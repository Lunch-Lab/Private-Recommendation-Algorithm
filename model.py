...
class BaseRankModel(object):
    ...
    def _init_tf_vars(self):
        with tf.name_scope(self.model_name):
            ...
            self.feature = tf.placeholder(
                tf.float32, shape=[None, self.params["feature_dim"]], name="feature"
            )
            self.training = tf.placeholder(tf.bool, shape=[], name="training")
            #### input for training
            self.label = tf.placeholder(tf.float32, shape=[None, 1], name="label")
            self.sorted_label = tf.placeholder(
                tf.float32, shape=[None, 1], name="sorted_label"
            )
    ...
    def _score_fn_inner(self, x, reuse=False):
        # deep
        hidden_units = [
            self.params["fc_dim"] * 4,
            self.params["fc_dim"] * 2,
            self.params["fc_dim"],
        ]
    ...
    def _get_derivative(self, score, Wk, lambda_ij, x):
        ...
        shape = tf.concat(
            [
                tf.shape(lambda_ij),
                tf.ones(
                    [tf.rank(dsi_dWk_minus_dsj_dWk) - tf.rank(lambda_ij)],
                    dtype=tf.int32,
                ),
            ],
            axis=0,
        ...
        grad = tf.reduce_mean(
            tf.reshape(lambda_ij, shape) * dsi_dWk_minus_dsj_dWk, axis=[0, 1]
        )
    ...
    def save_session(self):
        ...
        tf.train.write_graph(
            ...
            self.params["offline_model_dir"],
            "graph.pb",
            as_text=True,
        )
        self.saver.save(
            self.sess, self.params["offline_model_dir"] + "/model.checkpoint"
        )
    ...
    def fit(self, X, validation_data):
        ...
        loss_mean_train, err_mean_train, ndcg_mean_train, ndcg_all_mean_train = (
            self.evaluate(X)
        )
        if validation_data is not None:
            loss_mean_valid, err_mean_valid, ndcg_mean_valid, ndcg_all_mean_valid = (
        ...
        for epoch in range(self.params["epoch"]):
            ...
            batches = self._get_batch_index(
                train_idx_shuffle, self.params["batch_size"]
            )
            for i, idx in enumerate(batches):
                ...
                loss, lr, opt = self.sess.run(
                    (self.loss, self.learning_rate, self.train_op), feed_dict=feed_dict
                )
                total_loss = loss_decay * total_loss + (1.0 - loss_decay) * loss
                total_batch += 1
    ...
    def evaluate(self, X):
        ...
        for e, qid in enumerate(qid_unique):
            ...
            df = pd.DataFrame(
                {"label": X["label"][ind].flatten(), "score": score.flatten()}
            )
...
class DNN(BaseRankModel):
    ...
    def _build_model(self):
        ...
        logloss = tf.nn.sigmoid_cross_entropy_with_logits(
...
class RankNet(BaseRankModel):
    ...
    def _build_unfactorized_model(self):
        ...
        S_ij = tf.maximum(tf.minimum(1.0, S_ij), -1.0)
        P_ij = (1 / 2) * (1 + S_ij)
        s_i_minus_s_j = logits = score - tf.transpose(score)

        logloss = tf.nn.sigmoid_cross_entropy_with_logits(
            logits=s_i_minus_s_j, labels=P_ij
        )
    ...
    def _build_factorized_model(self):
        ...
        grads = [
            self._get_derivative(score, Wk, lambda_ij, self.feature) for Wk in vars
        ]
...
class LambdaRank(BaseRankModel):
    ...
    def _build_model(self):
        ...
        # current dcg
        index = tf.reshape(
            tf.range(1.0, tf.cast(self.batch_size, dtype=tf.float32) + 1),
            tf.shape(self.label),
        )
        cg_discount = tf.log(1.0 + index)
        rel = 2**self.label - 1
